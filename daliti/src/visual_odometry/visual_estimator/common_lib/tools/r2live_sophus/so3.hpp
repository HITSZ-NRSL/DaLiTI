...
class SO3 : public SO3Base<SO3<Scalar_, Options>> {
 public:
  ...
  /// of 1.
  ///
  SOPHUS_FUNC SO3(Transformation const& R,std::string caller=__builtin_FUNCTION()) : unit_quaternion_(R) {
    SOPHUS_ENSURE(isOrthogonal(R), "R is not orthogonal:\n %",
                  R * R.transpose());
    // if(!isOrthogonal(R)){
    //   scope_color(ANSI_COLOR_RED_BOLD);
    //   std::cout<<ANSI_COLOR_CYAN_BOLD<<"R from "<<caller<< " has some problems"<<ANSI_COLOR_RESET<<std::endl;
    // }
