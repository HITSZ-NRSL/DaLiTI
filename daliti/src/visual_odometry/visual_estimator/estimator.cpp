...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 8, 1>>>> &points,
                             const vector<float> &lidar_initialization_info,
                             const std_msgs::Header &header)
{
    // Add new image features
    if (f_manager.addFeatureCheckParallax(frame_count, points, td))
    ...
    ImageFrame imageframe(points, lidar_initialization_info, header.stamp.toSec());
...
int Estimator::refine_vio_system(StatesGroup state)
{
    ...
    // Rs[WINDOW_SIZE - 1] = state.rot_end;
    // Ps[WINDOW_SIZE - 1] = state.pos_end;
    // Vs[WINDOW_SIZE - 1] = state.vel_end;
    // // pre_integrations[WINDOW_SIZE - 1] = 

    Rs[WINDOW_SIZE] = state.rot_end;
    Ps[WINDOW_SIZE] = state.pos_end;
    Vs[WINDOW_SIZE] = state.vel_end;
    // pre_integrations[WINDOW_SIZE - 1] = 
    ...
    for (auto &it_per_id : estimator.f_manager.feature)
    {
        ...
        for (auto &it_per_frame : it_per_id.feature_per_frame)
        {
            ...
            if (imu_j == (frame_idx))
            {
                ...
                parameters_vec.push_back(estimator.para_Pose[imu_i]);
                parameters_vec.push_back(estimator.para_Pose[imu_j]);
                parameters_vec.push_back(estimator.para_Ex_Pose[0]);
                parameters_vec.push_back(estimator.para_Feature[feature_index]);
...
// StatesGroup Estimator::esikfUpdate(const StatesGroup &lio_state, const eigen_q &diff_vins_lio_q, const vec_3 &diff_vins_lio_t)
// {
//     StatesGroup state_vision_esikf = lio_state;
//     int flg_EKF_converged = 0;

//     double mean_reprojection_error = 0.0;
//     int minmum_number_of_camera_res = 10;
//     double deltaR = 0, deltaT = 0;
//     double CAM_MEASUREMENT_COV = 1e-3;
...
//     StatesGroup state_in = state_vision_esikf;
//     for (int iter_time = 0; iter_time < 2; iter_time++)
//     {

//         Eigen::Quaterniond q_pose_last = Eigen::Quaterniond(state_vision_esikf.rot_end * diff_vins_lio_q.inverse());
//         Eigen::Vector3d t_pose_last = state_vision_esikf.pos_end - diff_vins_lio_t;
//         this->para_Pose[WINDOW_SIZE][0] = t_pose_last(0);
//         this->para_Pose[WINDOW_SIZE][1] = t_pose_last(1);
//         this->para_Pose[WINDOW_SIZE][2] = t_pose_last(2);
//         this->para_Pose[WINDOW_SIZE][3] = q_pose_last.x();
//         this->para_Pose[WINDOW_SIZE][4] = q_pose_last.y();
//         this->para_Pose[WINDOW_SIZE][5] = q_pose_last.z();
//         this->para_Pose[WINDOW_SIZE][6] = q_pose_last.w();


//         construct_camera_measure(WINDOW_SIZE, *this, reppro_err_vec, J_mat_vec);


//         if (reppro_err_vec.size() < minmum_number_of_camera_res)
//         {
//             ROS_WARN("EKF failed, Size of reppro_err_vec: %d", reppro_err_vec.size());
//             // ROS_DEBUG("EKF failed, Size of reppro_err_vec: %d", reppro_err_vec.size());
...
//         K_1 = (H_T_H + (state_vision_esikf.cov / CAM_MEASUREMENT_COV).inverse()).inverse();
//         K = K_1.block<DIM_OF_STATES, 6>(0, 0) * Hsub_T;

//         auto vec = state_in - state_vision_esikf;
...
//         state_vision_esikf = state_in + solution;
...
//     {
//         // ROS_WARN("EKF failed, norm is big");
//         // ROS_INFO("EKF failed, norm is big");
//     }
//     else{
//         // ROS_INFO("EKF OK");
//     }
//     ROS_DEBUG("flg_EKF_converged: %d", flg_EKF_converged);
//     // return flg_EKF_converged ? state_vision_esikf : lio_state;
//     return lio_state;
// }

StatesGroup Estimator::optimizationUpdate(const StatesGroup &in_state, const bool &recv_lio_fail_flag)
{
    // in_state.display(in_state);
    StatesGroup current_state = in_state;
    ...
    {
        // printf("lio run valid \r");
        if (this->solver_flag == Estimator::SolverFlag::NON_LINEAR)
        {
            this->m_lio_state_prediction_vec[WINDOW_SIZE] = in_state;

            
            
            // diff_vins_lio_q = eigen_q(this->Rs[WINDOW_SIZE].transpose() * in_state.rot_end);
            // diff_vins_lio_t = in_state.pos_end - this->Ps[WINDOW_SIZE];
            ...
            if (1)
            {
                // ROS_DEBUG("----  vio-lio DIFF larger than 1m ----,diff is %lf m ", diff_vins_lio_t.norm());
                this->refine_vio_system(in_state);
                // diff_vins_lio_q.setIdentity();
                // diff_vins_lio_t.setZero();
            }
            // TicToc t_esikf;
            // current_state = esikfUpdate(in_state, diff_vins_lio_q, diff_vins_lio_t);
            // std::cout<<" esikf costed time: "<<t_esikf.toc()<<"ms"<<std::endl;
        }
    }
    else
    {
...
void Estimator::solve_image_pose(const std_msgs::Header &header)
{
    ...
    if (solver_flag == INITIAL)
    {
        if (frame_count == WINDOW_SIZE)
        {
            ...
            if (result)
            {
                ...
                ROS_INFO("solve_image_pose Initialization finish!");
    ...
    {
        TicToc t_solve_odometry;
        solveOdometry();
        // std::cout << ANSI_COLOR_CYAN_BOLD << "solve odometry time :  " << t_solve_odometry.toc() <<" ms\n" << ANSI_COLOR_RESET << std::endl;
        ...
        slideWindow();
        // std::cout << ANSI_COLOR_CYAN_BOLD << "slide window time :  " << t_slidewindow.toc() <<" ms" << ANSI_COLOR_RESET << std::endl;
        TicToc t_removefailure;
        f_manager.removeFailures();
        // std::cout << ANSI_COLOR_CYAN_BOLD << "remove failures time :  " << t_removefailure.toc() <<" ms" << ANSI_COLOR_RESET << std::endl;
...
StatesGroup Estimator::processData(const StatesGroup &in_state,
                                   const bool &recv_lio_fail_flag,
                                   const map<int, vector<pair<int, Eigen::Matrix<double, 8, 1>>>> &points,
                                   const vector<float> &lidar_initialization_info,
                                   const std_msgs::Header &header)
{
    StatesGroup current_state = in_state;
    // ? process Image
    // TicToc tictoc_process_image;
    processImage(points, lidar_initialization_info, header);
    // double t_process = tictoc_process_image.toc();
    // std::cout << ANSI_COLOR_CYAN_BOLD << "process time :  " << t_process <<" ms" << ANSI_COLOR_RESET << std::endl;
    // ? end process Image

    // ? added L-I-V upd
    // TicToc tictoc_optimization_update;
    current_state = optimizationUpdate(in_state, recv_lio_fail_flag);
    // double t_opt = tictoc_optimization_update.toc();
    // std::cout << ANSI_COLOR_CYAN_BOLD << "optimazation time :  " << t_opt <<" ms" << ANSI_COLOR_RESET << std::endl;
    // ? end L-I-V upd

    // ? solve image pose
    TicToc tictoc_solve;
    solve_image_pose(header);
    double t_solve = tictoc_solve.toc();
    // std::cout << ANSI_COLOR_CYAN_BOLD << "solve image pose time :  " << t_solve <<" ms" << ANSI_COLOR_RESET << std::endl;
    // std::cout << ANSI_COLOR_CYAN_BOLD << "____________________________ " << ANSI_COLOR_RESET << std::endl;
    // ? end solve image pose

    if (recv_lio_fail_flag)
    {
        current_state.pos_end << Ps[WINDOW_SIZE].x(), Ps[WINDOW_SIZE].y(), Ps[WINDOW_SIZE].z();
        current_state.vel_end << Vs[WINDOW_SIZE].x(), Vs[WINDOW_SIZE].y(), Vs[WINDOW_SIZE].z();
        current_state.rot_end = Rs[WINDOW_SIZE];
    }
...
{
    // ? LVI-SAM added : Lidar initialization
    {
        ...
        for (int i = 0; i <= WINDOW_SIZE; i++)
        {
                ...
                all_image_frame[Headers[i].stamp.toSec()].reset_id != all_image_frame[Headers[0].stamp.toSec()].reset_id)
            {
                ...
                ROS_WARN("Lidar initialization info not enough.");
                break;
            }
        }
...
void Estimator::solveOdometry()
{
    ...
    {
        TicToc tictoc_triangualate;
        f_manager.triangulate(Ps, tic, ric);
        // std::cout << ANSI_COLOR_CYAN_BOLD << "triangualate time :  " << tictoc_triangualate.toc() <<" ms" << ANSI_COLOR_RESET << std::endl;
        TicToc tictoc_optimzation;
        optimization();
        double optimization_time = tictoc_optimzation.toc();
        std::cout << ANSI_COLOR_CYAN_BOLD << "optimization time :  " << optimization_time <<" ms" << ANSI_COLOR_RESET << std::endl;

        std::ofstream file("~/daliti/src/DaLiTI/eskf_lio/Log/tio_optimization_time.txt", std::ios::app);
        if (file.is_open())
        {
            file << optimization_time << std::endl;
            file.close();
        }
